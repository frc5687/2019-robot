...
public class Stilt extends OutliersSubsystem {
    ...
    public void setLifterSpeed(double desiredSpeed) {
        double speed = desiredSpeed; // limit(desiredSpeed, isRetracted() ? 0 : -MAX_DOWN_SPEED , isExtended() ? 0 : MAX_UP_SPEED);
    ...
    public boolean isExtended() {
        return _extendedHall.get();// || (Math.abs(getPosition()-TOP_POSITION)<TOLERANCE);
    }

    public boolean isRetracted() {
        return _retractedHall.get();// || (Math.abs(getPosition()-BOTTOM_POSITION)<TOLERANCE);
    }
    ...
    public void updateDashboard() {
        ...
        metric("AtTop", isExtended());
        metric("AtBottom", isRetracted());
