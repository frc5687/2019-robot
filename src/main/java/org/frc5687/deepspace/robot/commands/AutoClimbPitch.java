...
public class AutoClimbPitch extends OutliersCommand {
    ...
    public AutoClimbPitch(Stilt stilt, Arm arm, DriveTrain driveTrain, CargoIntake cargoIntake, HatchIntake hatchIntake, Robot robot) {
        _stilt = stilt;
        _arm = arm;
    ...
    public void execute() {
        switch (_climbState) {
            ...
            case MoveArmAndStilt:
                ...
                double pitch = _robot.getIMU().getPitch();

                if (pitch < 0) {
                    double speed = Helpers.limit(pitch * CLIMB_PITCH_K, 0.1, 1.0);
                    _stilt.setLifterSpeed(speed);
                    metric("StiltSpeed", speed);
                } else {
                    error("Stopping stilt");
                    _stilt.setLifterSpeed(STILT_HIGH_HOLD_SPEED);
                ...
                if (_oi.isWheelieForwardPressed() ||
                        ...
                        && (_stilt.isExtended() || pitch >= CLIMB_PITCH_TOLERANCE))) {
                    metric("StiltSpeed", 0);
                    metric("ArmSpeed", 0);
                    _arm.setSpeed(0);
            ...
            case WheelieForward:
                _stilt.setLifterSpeed(STILT_HIGH_HOLD_SPEED);
                _stilt.setWheelieSpeed(WHEELIE_FORWARD_SPEED_HIGH);
                _driveTrain.disableBrakeMode();
            ...
            case LiftArm:
                ...
                if (_arm.getAngle() <= ARM_RETRACT_ANGLE) {
                    ...
                    _stiltTimeout = System.currentTimeMillis() + (_stilt.isExtended() ? STILT_TIMEOUT_HIGH : STILT_TIMEOUT_LOW);
                }
                break;
